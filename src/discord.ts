			...
			if (clipsChannel?.isTextBased()) {
				...
				if (lastMessage) {
					...
					const mostPopularClips =
						await this.twitchService.getTwitchClips()
...
function getClipMessageFromChat(
	...
	for (const [_id, message] of messages) {
		const [clipIdInMessage, _clipTitleInMessage] =
			getClipIdAndTitleFromMessage(clip, message)
...
function getClipIdAndTitleFromMessage(
	...
	const titleLine = subsString.find((line) =>
		line.startsWith(TITLE_LINE_START)
	)
